...
function MetadataFeaturesTable({ allFeaturesArray,sequenceLength }) {
  const featureCategories = allFeaturesArray?.reduce((curSet, ftr) => {
  ...
  const featureCategoriesAndColumnsJsx = Array.from(featureCategories).flatMap(
    (category, idx) => {
      ...
      return [  // 10 vh to config.js boxSizing:'border-box',height:'10vh', backgroundColor:'lightBlue'
        <div style={{display:'flex', flexWrap:'wrap', height:'4.5vh',backgroundColor:'lightblue',justifyContent:'center' , alignContent:'center'}}> 
          <h3 style={{ fontSize:'1.5vh', textAlign:'center'}}> {category.split("_").join(" ") }</h3> 
        </div>
         ,
  ...
  return (// can't give rowgap, because then the canvasses won't touch, and there will be dead zones for zoom, instead make canvas larger, and then not draw the figures to those areas
    <div style={{display:'grid',columnGap:'10px',gridTemplateColumns:'auto 1fr'}}>
